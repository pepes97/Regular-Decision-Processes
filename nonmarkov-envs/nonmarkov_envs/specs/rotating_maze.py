...
class RotatingMaze():
    ...
    def _compute_terminal_states(self):
        ...
        for state in self.STATES:
            if state[0] == self.GOAL_POSITION[0] and state[1] == self.GOAL_POSITION[1]:
                ...
                for observation in self.OBSERVATIONS:
                    ...
                    if self._transition_is_valid((prev_pos_x, prev_pos_y), (obs_pos_x, obs_pos_y), action, prev_orientation):
                        ...
                        if (obs_pos_x, obs_pos_y) == self.GOAL_POSITION:
                            theta[state][action][observation][self.REWARD_GOAL] = self.PROB_SUCCESS_MOVE
                        else:
                            theta[state][action][observation][self.REWARD_STEP] = self.PROB_SUCCESS_MOVE
                    ...
                    if self._transition_is_valid((prev_pos_x, prev_pos_y), (obs_pos_x, obs_pos_y), opposite_action, prev_orientation):
                        theta[state][action][observation] = dict()
                        if (obs_pos_x, obs_pos_y) == self.GOAL_POSITION:
                            theta[state][action][observation][self.REWARD_GOAL] = self.PROB_OPPOSITE_MOVE
                        else:
                            theta[state][action][observation][self.REWARD_STEP] = self.PROB_OPPOSITE_MOVE
        return theta
